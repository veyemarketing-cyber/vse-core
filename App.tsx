...
/** Calls your server route (/api/gemini) so the API key stays private. */
...
async function getOrchestrationPlan(input: string, mode: OrchestrationMode): Promise<SyncResult> {
  const prompt = `
You are the Velocity Sync Engine (VSE) Orchestration Planner.
...
- Provide at least 3 metrics.
- Provide at least 3 roadmap phases.
- triggeredActions can be [] if none.
`;
  ...
  try {
    return JSON.parse(jsonStr) as SyncResult;
  } catch {
    throw new Error(`Failed to parse orchestration JSON. Raw response:\n${text}`);
  }
}

async function scanVirginiaEvents(lat?: number, lng?: number): Promise<VirginiaEvent[]> {
  const prompt = `
You are VSE Regional Intel Scanner for Virginia (NOVA and Chesterfield focus).

Return ONLY valid JSON array (no markdown) matching:
Array<{
  "event_name": string,
  "date": string,
...
User coordinates (may be undefined):
lat: ${lat ?? 'undefined'}
lng: ${lng ?? 'undefined'}

Rules:
- Provide 6-10 entries.
- Keep source_url as plausible public URLs (no private links).
- Prefer procurement, chambers, government contracting, SWaM-related events.
`;
  ...
  try {
    return JSON.parse(jsonStr) as VirginiaEvent[];
  } catch {
    throw new Error(`Failed to parse events JSON. Raw response:\n${text}`);
  }
}
...
const App: React.FC = () => {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      const actions = (result as any)?.triggeredActions || [];
      if (Array.isArray(actions) && actions.length > 0) {
        actions.forEach((action: any) => {
          addLog(`ACTION: ${action.name}`, 'action', action.args);
        });
      }
      ...
      console.error(err);
      addLog(`ERROR: ${err?.message || 'Orchestration failed'}`, 'system');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEventScan = async () => {
    ...
    try {
      ...
      const data = await scanVirginiaEvents(coords.lat, coords.lng);
      setEvents(data);
      addLog(`Identified ${data.length} regional growth opportunities.`, 'info');
    } catch (err: any) {
      console.error(err);
      addLog(`Regional scan interrupted: ${err?.message || 'unknown error'}`, 'system');
    } finally {
      setScanningEvents(false);
    }
  };

  // ✅ Tailwind-safe mode config (NO dynamic bg-${color} strings)
  const modeConfig = {
    growth: {
      icon: <Layers size={24} />,
      label: 'Full Growth System',
      placeholder: 'Describe end-to-end business operations, traffic sources, and CRM pathways...',
      cardActive: 'bg-blue-600/10 border-blue-500/50 ring-1 ring-blue-500/30 glow-accent',
      iconActive: 'bg-blue-500 text-white shadow-lg'
    },
    seo: {
      icon: <Search size={24} />,
      label: 'SEO Orchestration',
      placeholder: 'Provide website audit data, technical health signals, or topical content gaps...',
      cardActive: 'bg-purple-600/10 border-purple-500/50 ring-1 ring-purple-500/30 glow-accent',
      iconActive: 'bg-purple-500 text-white shadow-lg'
    },
    crm: {
      icon: <Users size={24} />,
      label: 'CRM & Lifecycle',
      placeholder: 'Detail funnel stages, lead capture sources, and customer drop-off points...',
      cardActive: 'bg-emerald-600/10 border-emerald-500/50 ring-1 ring-emerald-500/30 glow-accent',
      iconActive: 'bg-emerald-500 text-white shadow-lg'
    }
  } as const;
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-[#050505] text-gray-200 font-inter">
      ...
      <aside className="w-64 border-r border-white/10 flex flex-col glass-panel">
        <div className="p-6">
          ...
          <nav className="space-y-1">
            <p className="px-4 py-2 text-[10px] font-bold text-gray-500 uppercase tracking-widest mb-1 text-blue-500/50">
              Master Intelligence
            </p>
            <SidebarItem
        ...
        <div className="flex-1">
          {activeTab === 'dashboard' && (
            <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
                ...
                <div className="glass-panel w-full max-w-3xl p-10 rounded-[2.5rem] border border-blue-500/30 relative overflow-hidden bg-gradient-to-b from-blue-600/10 to-transparent shadow-[0_20px_50px_rgba(59,130,246,0.15)] glow-accent group transition-all duration-500 hover:border-blue-500/50">
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    <div className="flex items-center gap-2 mb-6">
                      ...
                      <span className="px-4 py-1.5 bg-blue-600/20 text-blue-400 text-[11px] font-black uppercase tracking-[0.4em] rounded-full border border-blue-500/20 shadow-lg">
                        Daily Dedication
                      </span>
                      <div className="h-px w-8 bg-blue-500/50"></div>
                    </div>
                    ...
                    <div className="w-full max-w-xl p-8 rounded-3xl bg-black/40 border border-white/5 backdrop-blur-md shadow-inner relative group/scripture">
                      <div className="flex flex-col items-center">
                        <p className="text-[10px] text-blue-500 uppercase font-black tracking-[0.3em] mb-4 opacity-70">
              ...
              <header className="flex justify-between items-start pt-8 border-t border-white/5">
                <div>
                  ...
                  <p className="text-gray-400 text-lg font-medium">
                    Lead Architect Interface: Initializing regional growth protocols.
                  </p>
                </div>
              ...
              {/* Mode Selector (Tailwind-safe) */}
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {Object.entries(modeConfig).map(([key, config]) => {
                  const isActive = selectedMode === (key as OrchestrationMode);
                  ...
                  return (
                    <button
                      ...
                      className={`p-8 rounded-[2rem] border transition-all text-left relative overflow-hidden group ${
                        isActive ? config.cardActive : 'bg-white/5 border-white/10 hover:border-white/20'
                      }`}
                    >
                      <div
                        className={`mb-4 p-4 inline-block rounded-2xl ${
                          isActive ? config.iconActive : 'bg-white/10 text-gray-400'
                        }`}
                      >
                        {config.icon}
                      </div>
                      ...
                      <p className="text-sm text-gray-500 leading-relaxed font-medium">
                        Agentic {key.toUpperCase()} systems design and regional coordination.
                      </p>
                      {isActive && (
                    ...
                    <div className="flex items-center gap-4 mb-6 relative z-10">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 italic font-medium">
                          Active Protocol: {modeConfig[selectedMode].label}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-y-auto space-y-4 font-mono text-[11px] pr-2 custom-scrollbar">
                      {statusLog.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-gray-700 opacity-50 space-y-6">
                          ...
                          <p className="uppercase tracking-[0.4em] text-center px-12 leading-relaxed">
                            Awaiting Master Signal Synchronization
                          </p>
                        </div>
                      ) : (
                        statusLog.map((log) => (
                          <div
                            key={log.id}
                            className={`p-4 rounded-2xl border leading-tight transition-all duration-300 animate-in fade-in slide-in-from-right-2 ${
                              log.type === 'action'
                          ...
                          >
                            <div className="flex justify-between items-center mb-2">
                              ...
                              <span className="text-[9px] uppercase font-black opacity-60 tracking-[0.2em] px-2 py-0.5 rounded bg-black/20">
                                [{log.type}]
                              </span>
                            </div>
              ...
              {!syncResult ? (
                <div className="flex flex-col items-center justify-center h-[70vh] text-center glass-panel rounded-[3rem] p-16 shadow-2xl bg-black/40">
                  ...
                  <h3 className="text-3xl font-black text-white mb-4 tracking-tight">
                    Awaiting Master Intelligence Stream
                  </h3>
                  <p className="text-gray-500 max-w-sm font-medium text-lg leading-relaxed mb-10">
                    No active orchestration plan detected. Run a synchronization task to initialize the Master Architect.
                  </p>
                  <button
                    onClick={() => setActiveTab('dashboard')}
                ...
                <>
                  <header className="flex justify-between items-end border-b border-white/10 pb-10">
                    <div>
                      <div className="flex items-center gap-3 mb-4">
                        <span
                          className={`px-4 py-1.5 rounded-full text-[11px] font-black uppercase tracking-[0.2em] shadow-lg ${
                            selectedMode === 'growth'
                      ...
                      <p className="text-gray-400 text-xl font-medium italic">
                        Integrated VSE-Master protocols for 2026 regional dominance.
                      </p>
                    </div>
                    <div className="text-right">
                      <div className="text-xs font-mono text-gray-500 mb-2 uppercase tracking-[0.2em] font-black">
                        VSE Protocol Identifier
                      </div>
                  ...
                  {/* Triggered Actions */}
                  {(syncResult as any)?.triggeredActions?.length > 0 && (
                    <div className="glass-panel p-8 rounded-[2.5rem] border-l-8 border-l-emerald-500 bg-black/40 shadow-2xl overflow-hidden relative group">
                      ...
                      <div className="flex justify-between items-center mb-8 relative z-10">
                        ...
                        <div className="flex items-center gap-3 px-4 py-1.5 bg-emerald-500/10 rounded-full border border-emerald-500/20 text-[11px] text-emerald-400 font-black tracking-widest animate-pulse">
                          <Webhook size={14} /> ACTION QUEUE
                        </div>
                      </div>

                      <div className="space-y-4 font-mono relative z-10">
                        {(syncResult as any).triggeredActions.map((action: any, idx: number) => (
                          <div
                            key={action?.id || idx}
                            className="p-6 bg-white/5 rounded-[1.5rem] border border-white/10 text-sm hover:border-emerald-500/30 transition-all hover:bg-white/[0.07] shadow-lg"
                          >
                            <div className="flex justify-between items-center mb-4">
                              <span className="text-emerald-500 font-black text-lg">PROTOCOL: {action.name}()</span>
                              <span className="text-gray-600 font-black opacity-50 tracking-tighter bg-black/20 px-3 py-1 rounded-lg uppercase">
                                Node: {action.id || 'n/a'}
                              </span>
                            </div>
                    ...
                    <div className="lg:col-span-5 space-y-10">
                      <div className="glass-panel p-8 rounded-[2.5rem] border-l-8 border-l-blue-600 bg-black/40 shadow-xl">
                        ...
                        <div className="grid grid-cols-1 gap-4">
                          {Object.entries(syncResult.diagnosis?.signals || {}).map(([key, value]) =>
                            value ? (
                              <div
                                key={key}
                                className="bg-white/5 p-5 rounded-2xl border border-white/5 group hover:border-blue-500/40 transition-all cursor-default shadow-sm"
                              >
                                <p className="text-[11px] text-blue-500 uppercase font-black tracking-[0.2em] mb-2">
                                  {key}
                                </p>
                                <p className="text-md text-gray-300 group-hover:text-white transition-colors leading-relaxed font-medium">
                                  {String(value)}
                                </p>
                              </div>
                            ) : null
                      ...
                      <div className="glass-panel p-8 rounded-[2.5rem] border-l-8 border-l-red-600 bg-black/40 shadow-xl">
                        ...
                        <div className="space-y-4">
                          {(syncResult.diagnosis?.inefficiencies || []).map((item, idx) => (
                            <div
                              key={idx}
                              className="flex items-start gap-4 p-5 bg-red-500/5 rounded-2xl border border-red-500/10 text-md text-gray-300 hover:bg-red-500/10 transition-all shadow-sm"
                            >
                    ...
                    <div className="lg:col-span-7">
                      <div className="glass-panel p-10 rounded-[3rem] h-full flex flex-col border border-white/10 bg-black/40 shadow-2xl">
                        <div className="flex justify-between items-center mb-10">
                          ...
                          <p className="text-[11px] text-gray-500 font-mono uppercase tracking-[0.4em] font-black">
                        ...
                        <div className="flex-1 min-h-[450px]">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={syncResult.metrics || []} layout="vertical" margin={{ left: 20 }}>
                              ...
                              <YAxis dataKey="label" type="category" stroke="#999" fontSize={14} width={130} />
                              <Tooltip
                              ...
                              <Bar dataKey="value" radius={[0, 8, 8, 0]} barSize={32}>
                                {(syncResult.metrics || []).map((_, index) => (
                                  <Cell
                                    key={`cell-${index}`}
                  ...
                  <div className="space-y-8 pt-12">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-gray-500 font-bold uppercase tracking-[0.1em] text-sm">
                          VSE Core Architecture Nodes
                        </p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                      {(syncResult.architecture?.agents || []).map((agent: any) => (
                        <div
                          key={agent.id}
                          className="p-10 bg-white/5 border border-white/10 rounded-[3rem] hover:border-indigo-500/50 transition-all group relative overflow-hidden backdrop-blur-xl shadow-xl"
                        >
                          <div className="flex justify-between items-start mb-6 relative z-10">
                            <h4 className="text-2xl font-black text-white group-hover:text-indigo-400 transition-colors tracking-tight leading-none uppercase">
                              {agent.name}
                            </h4>
                          ...
                          <p className="text-md text-gray-400 mb-10 leading-relaxed min-h-[100px] font-medium relative z-10">
                            {agent.description}
                          </p>
                          <div className="space-y-4 relative z-10">
                            <p className="text-[10px] text-gray-500 uppercase font-black tracking-[0.3em] mb-2">
                              Architectural Logic Path
                            </p>
                            <div className="flex flex-wrap gap-3">
                              {(agent.dependencies || []).map((dep: string) => (
                                <span
                                  key={dep}
                                  className="text-[11px] bg-indigo-500/10 text-indigo-300 px-4 py-1.5 rounded-full border border-indigo-500/10 font-black shadow-sm"
                                >
                                  {dep}
                                </span>
                              ))}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-3xl font-black text-white tracking-tight uppercase">
                          Master Phase Deployment
                        </h3>
                        <p className="text-gray-500 font-bold uppercase tracking-[0.1em] text-sm">
                          Strategic Regional Sequence
                        </p>
                      </div>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                      {(syncResult.roadmap || []).map((phase: any, idx: number) => (
                        <div
                          key={idx}
                          className="glass-panel p-10 rounded-[3.5rem] relative flex flex-col h-full border-t-[12px] border-t-blue-500/50 shadow-[0_30px_60px_rgba(0,0,0,0.5)] bg-black/60 transition-all hover:-translate-y-2 duration-500"
                        >
                          <div className="flex justify-between items-start mb-8">
                            <div className="bg-blue-600 text-white text-[11px] font-black px-5 py-1.5 rounded-full shadow-2xl uppercase tracking-[0.1em]">
                              PHASE 0{idx + 1}
                            </div>
                            <span
                              className={`text-[10px] px-4 py-1.5 rounded-full font-black uppercase tracking-[0.2em] border-2 shadow-inner ${
                                phase.priority === 'High'
                          ...
                          <h4 className="text-3xl font-black text-white mb-10 uppercase tracking-tighter leading-none border-b border-white/5 pb-6">
                            {phase.phase}
                          </h4>
                          ...
                          <ul className="space-y-6 flex-1">
                            {(phase.items || []).map((item: string, i: number) => (
                              <li
                                key={i}
                                className="flex items-start gap-5 text-gray-300 text-md leading-relaxed group cursor-default"
                              >
                          ...
                          <div className="mt-14 pt-10 border-t border-white/5 flex justify-between items-center">
                            <div>
                              <p className="text-[11px] text-gray-600 font-black uppercase tracking-[0.3em] mb-2">
                                Automation Layer
                              </p>
          ...
          {activeTab === 'events' && (
            <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-right-4 duration-700">
              <header className="flex justify-between items-end border-b border-white/10 pb-12">
                <div>
                  <h2 className="text-5xl font-black text-white mb-4 tracking-tighter underline decoration-blue-500/40">
                    Regional Intelligence
                  </h2>
                  <p className="text-gray-400 text-xl font-medium max-w-2xl">
                    Targeting Chesterfield and Northern Virginia procurement hubs for strategic SWaM integration.
                  </p>
                </div>
                <button
              ...
              {events.length === 0 ? (
                <div className="glass-panel p-32 rounded-[5rem] text-center flex flex-col items-center border-dashed border-4 border-white/5 bg-black/40 shadow-inner">
                  ...
                  <h3 className="text-4xl font-black text-white mb-6 tracking-tight uppercase italic">
                    Intelligence Node Offline
                  </h3>
                  <p className="text-gray-500 max-w-xl mx-auto leading-relaxed text-xl font-medium">
                    Trigger a regional scan to identify SWaM-eligible events and procurement pathways in Northern Virginia and Chesterfield.
                  </p>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10 pb-40">
                  {events.map((event, idx) => (
                    <div
                      key={idx}
                      className="glass-panel p-12 rounded-[3.5rem] group border-l-[12px] border-l-indigo-600/30 hover:border-l-indigo-400 transition-all hover:bg-white/[0.04] shadow-2xl relative overflow-hidden backdrop-blur-xl"
                    >
                      ...
                      <div className="flex justify-between items-start mb-8 relative z-10">
                        ...
                        <span
                          className={`text-[11px] px-5 py-2 rounded-full font-black uppercase tracking-[0.2em] border-2 shadow-md ${
                            event.priority_level === 'High'
                              ? 'bg-red-500/10 text-red-500 border-red-500/30'
                              : event.priority_level === 'Medium'
                      ...
                      <h4 className="text-4xl font-black text-white mb-4 group-hover:text-indigo-400 transition-colors leading-[1.05] tracking-tighter relative z-10 uppercase">
                        {event.event_name}
                      </h4>
                      ...
                      <div className="bg-black/60 p-10 rounded-[2.5rem] mb-10 border border-white/5 backdrop-blur-sm shadow-[inset_0_4px_30px_rgba(0,0,0,0.5)] relative z-10 group/assessment hover:border-white/10 transition-colors">
                        <p className="text-[11px] text-indigo-400 font-black mb-6 uppercase tracking-[0.4em] font-mono border-b border-indigo-500/20 pb-4">
                          Architect Assessment Node
                        </p>
                        <p className="text-lg text-gray-200 italic leading-relaxed font-medium">
                          "{event.strategic_reason}"
                        </p>
                      </div>
      ...
      <style>{`
        ...
        .glow-accent { box-shadow: 0 0 50px rgba(59, 130, 246, 0.1); }
      `}</style>
    </div>
  );
};
...
const SidebarItem: React.FC<{
  ...
  >
    ...
    <span className={`${active ? 'text-blue-400' : 'text-gray-600 group-hover:text-gray-400'} transition-all duration-300 scale-110`}>
      {icon}
    </span>
...
const StatCard: React.FC<{ title: string; value: string; sub: string; icon: React.ReactNode }> = ({
  ...
  <div className="glass-panel p-10 rounded-[3rem] border border-white/5 group hover:border-blue-500/30 transition-all hover:bg-white/[0.03] shadow-2xl relative overflow-hidden">
    <div className="flex justify-between items-start mb-8 relative z-10">
      <div className="p-4 bg-white/5 rounded-[1.25rem] border border-white/5 group-hover:bg-blue-600/10 group-hover:border-blue-500/20 transition-all shadow-inner">
        {icon}
      </div>
      <p className="text-[11px] text-gray-500 font-mono uppercase tracking-[0.3em] font-black text-nowrap group-hover:text-blue-400 transition-colors">
        {title}
      </p>
    </div>
    <p className="text-5xl font-black text-white mb-3 tracking-tighter leading-none text-nowrap group-hover:scale-105 transition-transform origin-left">
      {value}
    </p>
